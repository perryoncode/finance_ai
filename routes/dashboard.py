...
from services.finance_tools import normalize_csv, to_transactions
...
def dashboard_page():
    ...
    return render_template("dashboard.html",
                           user=session["user"],
                           txs=txs, income=income, expense=expense)
...
def upload_csv():
    ...
    pg = get_pg_client_or_redirect()
    if not isinstance(pg, type(authed_postgrest(None))):
        return pg  # redirect if session expired

    # upsert categories & insert transactions
    for r in rows:
